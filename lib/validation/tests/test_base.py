...
class MockRule1(ValidationRule):
    async def __call__(
        ...
        if value == "invalid":
            raise InputError("Invalid value", path=path, message="Value is invalid", code="invalid_value")
...
class MockRule2(ValidationRule):
    async def __call__(
        ...
        if value != 123:
            raise InputError("Invalid value", path=path, message="Value is not 123", code="invalid_value")
...
def test_validate_failure() -> None:
    ...
    assert exc_info.value.exceptions[0].message == "Value is invalid"
...
def test_validate_multiple_failures() -> None:
    ...
    assert any(error.message == "Value is invalid" for error in exc_info.value.exceptions)
    assert any(error.message == "Value is not 123" for error in exc_info.value.exceptions)
